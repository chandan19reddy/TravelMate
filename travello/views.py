...
def index(request):
    dests = Destination.objects.all()[::-1][:6]
    dest1 = Detailed_desc.objects.all()[::-1]
...
class KeyValueForm(forms.Form):
    ...
    last_name = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control w-100', 'placeholder': 'Last Name'})
    )
    age = forms.IntegerField(
        widget=forms.NumberInput(attrs={'class': 'form-control w-100', 'placeholder': 'Age'})
    )
